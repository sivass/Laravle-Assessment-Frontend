...
const ProductCart = (props) => {
  ...
  return (
    <div className='bg-white p-5 rounded-xl shadow-sm'>
       ...
       <span class={`me-2 rounded  px-2.5 py-0.5 text-xs font-medium  uppercase ${type === "physical" ? "bg-blue-300 text-blue-800" : " bg-slate-300 text-slate-800"}`}> {type} </span>
       <h3 className='text-lg font-semibold text-gray-900 py-2'>{name}</h3>
       <div className='flex flex-col'>
            <p className='text-xs font-normal uppercase'>SKU : <span className='font-bold'>{sku}</span></p>
       </div>
